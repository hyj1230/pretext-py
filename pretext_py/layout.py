...
def measureAnalysis(
  ...
  letterSpacing: int,
    ...
    lineEndPaintAdvances = []
    kinds = []
    simpleLineWalkFastPath = len(analysis['chunks']) <= 1 and not hasLetterSpacing
    segStarts = [] if includeSegments else None
    breakableFitAdvances = []
    breakablePreferredBreaks = []
    spacingGraphemeCounts = []
    segments = [] if includeSegments else None
    preparedStartByAnalysisIndex = [None] * analysis['len']
    ...
    def pushMeasuredTextSegment(
      ...
      allowOverflowBreaks: bool,
        ...
        textMetrics = getSegmentMetrics(str(text), cache)
    ...
    for mi in range(analysis['len']):
        preparedStartByAnalysisIndex[mi] = len(widths)
        ...
        if segKind == 'text' and segMetrics['containsCJK']:
            ...
            for i in range(len(measuredUnits)):
                ...
                pushMeasuredTextSegment(
                  ...
                  segStart + unit['start'],
                  segWordLike,
                  wordBreak == 'keep-all' or not isCJK(unit['text']),
                )
            continue
    ...
    chunks = mapAnalysisChunksToPreparedChunks(analysis['chunks'], preparedStartByAnalysisIndex, len(widths))
...
def mapAnalysisChunksToPreparedChunks(
  chunks,
  preparedStartByAnalysisIndex: list,
  preparedEndSegmentIndex,
):
    preparedChunks = []
    for i in range(len(chunks)):
        chunk = chunks[i]
        startSegmentIndex = preparedStartByAnalysisIndex[chunk['startSegmentIndex']] if chunk['startSegmentIndex'] < len(preparedStartByAnalysisIndex) else preparedEndSegmentIndex
        endSegmentIndex = preparedStartByAnalysisIndex[chunk['endSegmentIndex']] if chunk['endSegmentIndex'] < len(preparedStartByAnalysisIndex) else preparedEndSegmentIndex
        consumedEndSegmentIndex = preparedStartByAnalysisIndex[chunk['consumedEndSegmentIndex']] if chunk['consumedEndSegmentIndex'] < len(preparedStartByAnalysisIndex) else preparedEndSegmentIndex
