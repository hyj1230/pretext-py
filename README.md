...
Due to technical reasons, **setting `locales` is not currently supported**. Therefore, we additionally provide a way to customize the `Segmenter`; see [url](url).
If you have any information on fully implementing `Intl.Segmenter`, please let me know.

### measureText

Only a simple implementation using `pygame` is provided in `pretext_py/py_canvas`. Therefore, when using this project, you should also replace the `measureText` implementation; see [url](url).
...
Currently only one demo has been ported: [dragon_fast.py](url). Due to font and `pygame` limitations, some characters (e.g., Arabic and emoji) may not render correctly.
...
由于技术原因，**暂不支持 `locales` 的设置**，因此我们额外提供了自定义 `Segmenter` 的方法，详见 [url](url)。
如果你有关于完整实现 `Intl.Segmenter` 的任何信息，欢迎告知我。

### measureText

仅在 `pretext_py/py_canvas` 中使用 `pygame` 进行了简单实现，因此在使用本项目时，您也应替换 `measureText` 的实现，详见 [url](url)。
...
目前仅还原了一个示例：[dragon_fast.py](url)。由于字体和 `pygame` 的限制，某些文字（如阿拉伯语和 emoji）可能无法正常渲染。
